...
class Solution(object):
    def leastInterval(self, tasks, n):
        ...
        #dic=dict(sorted(dic.items(),key=lambda e:e[1],reverse=True))
        #其实是每一轮需要找出字典中value值在前n+1的项
        #n=len(tasks)
        # while(len(dic)!=0):
        # 	newans=ans+max(0,n+1-len(dic))
        # 	index=0
        ...
        # 	if(len(dic)!=0):
        # 		ans=newans
        #不是排序，而是大顶堆     python没有大顶堆，用小顶堆取负实现...
        heap = [-c for c in dic.values()]
        heapq.heapify(heap)
        while heap:
        	stack=[]
        	cnt=0
